...
void memcpy_by_audio_format(void* dst, audio_format_t dst_format, const void* src, audio_format_t src_format, size_t count)
{
    ...
    LOG_ALWAYS_FATAL("invalid src format %#x for dst format %#x", src_format, dst_format);
}
...
{
    const audio_channel_representation_t src_representation = audio_channel_mask_get_representation(src_channel_mask);
    const audio_channel_representation_t dst_representation = audio_channel_mask_get_representation(dst_channel_mask);
    ...
    switch (src_representation)
    {
        case AUDIO_CHANNEL_REPRESENTATION_POSITION:
            ...
            {
                case AUDIO_CHANNEL_REPRESENTATION_POSITION:
                    return memcpy_by_index_array_initialization(idxary, arysize, dst_bits, src_bits);
                case AUDIO_CHANNEL_REPRESENTATION_INDEX:
                    return memcpy_by_index_array_initialization_dst_index(idxary, arysize, dst_bits, src_bits);
                default:
                    return 0;
            }
            break;
        case AUDIO_CHANNEL_REPRESENTATION_INDEX:
            ...
            {
                case AUDIO_CHANNEL_REPRESENTATION_POSITION:
                    return memcpy_by_index_array_initialization_src_index(idxary, arysize, dst_bits, src_bits);
                case AUDIO_CHANNEL_REPRESENTATION_INDEX:
                    return memcpy_by_index_array_initialization(idxary, arysize, dst_bits, src_bits);
                default:
                    return 0;
