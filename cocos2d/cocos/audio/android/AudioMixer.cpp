...
namespace cocos2d
{
    namespace experimental
    {
        ...
        bool AudioMixer::setChannelMasks(int name, audio_channel_mask_t trackChannelMask, audio_channel_mask_t mixerChannelMask)
        {
            ...
            ALOGE_IF(status != OK, "prepareForDownmix error %d, track channel mask %#x, mixer channel mask %#x", status, track.channelMask,
         ...
         */
        static inline bool setVolumeRampVariables(float newVolume, int32_t ramp, int16_t* pIntSetVolume, int32_t* pIntPrevVolume, int32_t* pIntVolumeInc,
        ...
        void AudioMixer::setParameter(int name, int target, int param, void* value)
        {
            ...
            switch (target)
            {
                ...
                case VOLUME:
                    ...
                    {
                        case AUXLEVEL:
                            if (setVolumeRampVariables(*reinterpret_cast<float*>(value), target == RAMP_VOLUME ? mState.frameCount : 0, &track.auxLevel,
                                                       &track.prevAuxLevel, &track.auxInc, &track.mAuxLevel, &track.mPrevAuxLevel, &track.mAuxInc))
                            {
                        ...
                        default:
                            ...
                            {
                                if (setVolumeRampVariables(*reinterpret_cast<float*>(value), target == RAMP_VOLUME ? mState.frameCount : 0,
                                                           &track.volume[param - VOLUME0], &track.prevVolume[param - VOLUME0],
                                                           &track.volumeInc[param - VOLUME0], &track.mVolume[param - VOLUME0],
                                                           &track.mPrevVolume[param - VOLUME0], &track.mVolumeInc[param - VOLUME0]))
                                {
                                    ALOGV("setParameter(%s, VOLUME%d: %04x)", target == VOLUME ? "VOLUME" : "RAMP_VOLUME", param - VOLUME0,
                        ...
                        case PLAYBACK_RATE:
                        {
                            ...
                            if (track.setPlaybackRate(*playbackRate))
                            {
                                ALOGV("setParameter(TIMESTRETCH, PLAYBACK_RATE, STRETCH_MODE, FALLBACK_MODE "
                                      "%f %f %d %d",
                                      playbackRate->mSpeed, playbackRate->mPitch, playbackRate->mStretchMode, playbackRate->mFallbackMode);
                                // invalidateState(1 << name);
                            }
                        }
                        break;
        ...
        void AudioMixer::process__validate(state_t* state, int64_t pts)
        {
                    ...
                    if (all16BitsStereoNoResample && !volumeRamp)
                    {
                        if (countActiveTracks == 1)
                        {
                            ...
                            if ((t.needs & NEEDS_MUTE) == 0)
                            {
                                ...
                                state->hook = getProcessHook(PROCESSTYPE_NORESAMPLEONETRACK, t.mMixerChannelCount, t.mMixerInFormat, t.mMixerFormat);
                            }
                        }
                    }
                }
            ...
            ALOGV("mixer configuration change: %d activeTracks (%08x) "
                  ...
                  countActiveTracks, state->enabledTracks, all16BitsStereoNoResample, resampling, volumeRamp);
                ...
                do
                {
                    ...
                    while (e2)
                    {
                        ...
                        while (outFrames)
                        {
                            ...
                            if (inFrames > 0)
                            {
                                t.hook(&t, outTemp + (BLOCKSIZE - outFrames) * t.mMixerChannelCount, inFrames, state->resampleTemp, aux);
                ...
                while (e1)
                {
                    ...
                    else
                    {
                        ...
                        while (outFrames < numFrames)
                        {
                            ...
                            t.hook(&t, outTemp + outFrames * t.mMixerChannelCount, t.buffer.frameCount, state->resampleTemp, aux);
...
#define MIXTYPE_MONOVOL(mixtype) \
    (mixtype == MIXTYPE_MULTI ? MIXTYPE_MULTI_MONOVOL : mixtype == MIXTYPE_MULTI_SAVEONLY ? MIXTYPE_MULTI_SAVEONLY_MONOVOL : mixtype)
        ...
        static void volumeRampMulti(uint32_t channels, TO* out, size_t frameCount, const TI* in, TA* aux, TV* vol, const TV* volinc, TAV* vola, TAV volainc)
        {
            switch (channels)
            {
                case 1:
        ...
        void AudioMixer::volumeMix(TO* out, size_t outFrames, const TI* in, TA* aux, bool ramp, AudioMixer::track_t* t)
        {
            if (USEFLOATVOL)
            {
                if (ramp)
                {
                    volumeRampMulti<MIXTYPE>(t->mMixerChannelCount, out, outFrames, in, aux, t->mPrevVolume, t->mVolumeInc, &t->prevAuxLevel, t->auxInc);
                    if (ADJUSTVOL)
                    {
            ...
            else
            {
                if (ramp)
                {
                    volumeRampMulti<MIXTYPE>(t->mMixerChannelCount, out, outFrames, in, aux, t->prevVolume, t->volumeInc, &t->prevAuxLevel, t->auxInc);
                    if (ADJUSTVOL)
                    {
                        t->adjustVolumeRamp(aux != NULL);
                    }
         ...
         */
        AudioMixer::hook_t AudioMixer::getTrackHook(int trackType, uint32_t channelCount, audio_format_t mixerInFormat, audio_format_t mixerOutFormat __unused)
        {
            ...
            switch (trackType)
            {
                ...
                case TRACKTYPE_RESAMPLE:
                    switch (mixerInFormat)
                    {
                        ...
                        case AUDIO_FORMAT_PCM_16_BIT:
                            return (AudioMixer::hook_t)track__Resample<MIXTYPE_MULTI, int32_t, int16_t, int32_t>;
                        default:
                            LOG_ALWAYS_FATAL("bad mixerInFormat: %#x", mixerInFormat);
                            break;
                    }
         ...
         */
        AudioMixer::process_hook_t AudioMixer::getProcessHook(int processType, uint32_t channelCount, audio_format_t mixerInFormat, audio_format_t mixerOutFormat)
        {
