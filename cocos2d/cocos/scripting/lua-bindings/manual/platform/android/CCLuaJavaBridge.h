...
public:
     ...
     * @return the reference count of the functionId if luaj_function_id_retain table exists and the corresponding value for functionId exists, otherwise return
     * 0.
     *
     * @lua NA
     * @js NA
    ...
    /**
     * Release a reference count for the Lua functionId, If the reference count is still greater than 0,save this reference in the Lua table named
     * luaj_function_id_retain. Otherwise, remove the reference about this functionId in the luaj_function_id table and the luaj_function_id_retain table by set
     * the corresponding value nil.
     *
     * @param functionId the id of Lua function.
     * @return the reference count of the functionId if the luaj_function_id table, the luaj_function_id_retain table and the corresponding value for functionId
     * exists a reference count for the Lua functionId is still greater than 0, and otherwise return 0.
     *
     * @lua NA
     * @js NA
     */
     ...
     * @return a number value returned from the Lua function when call successfully, otherwise return -1 or the opposite number for one of the three numbers
     * LUA_ERRRUN, LUA_ERRMEM and LUA_ERRERR.
     *
     * @lua NA
     * @js NA
     */
...
private:
    ...
    public:
        ...
        int getErrorCode(void) { return m_error; }

        JNIEnv* getEnv(void) { return m_env; }
