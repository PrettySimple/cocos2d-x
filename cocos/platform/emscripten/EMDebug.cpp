...
Attempts to detect memory heap overflow/underflow and use-after-free (well, doesn't detect, but corrupts the memory bad enough to hopefully make the program crash
or misbehave ASAP).
...
TODO:
	...
	[-] Get rid of retain %, but instead always retain (when requested), and forcibly free when malloc() fails for internal structures or safe mode (requires non-throwing malloc)
...
/*
	...
	Untracked wrappers, as their name suggests, don't track allocations. However, they do release retained tracked allocations under memory pressure (when emscripten_builtin_malloc() fails)
...
#define	__assert(expr) do { ++__in_safe_mode; { assert((expr)); } --__in_safe_mode; } while(0)
...
*/
	...
	// "Rebind" constructor
	template <typename U>
	sanitizer_allocator(const sanitizer_allocator<U>& alloc) noexcept { (void)alloc; }
...
using alloc_map_type = std::unordered_map<uintptr_t, alloc_t, std::hash<uintptr_t>, std::equal_to<uintptr_t>, sanitizer_allocator<std::pair<const uintptr_t, alloc_t>>>;
...
static bool	__check_overflow(const alloc_t& entry)
{
	...
	for(uint8_t	*tmp = reinterpret_cast<uint8_t *>(entry.ptr) + entry.size; tmp < reinterpret_cast<uint8_t *>(entry.ptr + entry.size + (sizeof(uintptr_t) * SANITIZE_SIZE)); ++tmp)
	{
		if(*tmp != SANITIZE_PATTERN)
			return true;
	}
...
static bool	__check_retained(const retain_t& entry)
{
	...
	for(uint8_t *tmp = reinterpret_cast<uint8_t *>(entry.ptr - (sizeof(uintptr_t) * SANITIZE_SIZE)); tmp < reinterpret_cast<uint8_t *>(entry.ptr + entry.size + (sizeof(uintptr_t) * SANITIZE_SIZE)); ++tmp)
	{
		if(*tmp != SANITIZE_PATTERN)
			return true;
	}
...
static inline void	__unretain_all()
{
	for(size_t i = 0; i < __retain_list().count; ++i)
	{
		...
		if(
			!entry.write_after_free_reported &&
			__check_retained(entry)
		)
...
void * __attribute__((noinline)) malloc(size_t size)
{
	...
	if(
		!__alloc_list().emplace(
			std::piecewise_construct,
			std::forward_as_tuple(result_ptr),
			std::forward_as_tuple(result_ptr, size)
		).second
	)
	{
