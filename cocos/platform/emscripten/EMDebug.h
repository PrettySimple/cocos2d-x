...
public:
    ...
    static void assert_ptr(uintptr_t ptr, const char* file, int line) noexcept { assert_ptr(reinterpret_cast<void*>(ptr), file, line); }
...
{
public:
    EmscriptenSticky(const char* name) noexcept { printf("\n[STICKY]:%s\n", name); }

    ~EmscriptenSticky() noexcept { printf("\n[/STICKY]\n"); }
};
