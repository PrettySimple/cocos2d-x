...
namespace cocostudio
{
    ...
    void TextReader::setPropsFromBinary(cocos2d::ui::Widget* widget, CocoLoader* cocoLoader, stExpCocoNode* cocoNode)
    {
        ...
        for (int i = 0; i < cocoNode->GetChildNum(); ++i)
        {
            ...
            else if (key == P_AreaWidth) { label->setTextAreaSize(Size(valueToFloat(value), label->getTextAreaSize().height)); }
            else if (key == P_AreaHeight) { label->setTextAreaSize(Size(label->getTextAreaSize().width, valueToFloat(value))); }
    ...
    Offset<Table> TextReader::createOptionsWithFlatBuffers(const tinyxml2::XMLElement* objectData, flatbuffers::FlatBufferBuilder* builder)
    {
        ...
        auto options = CreateTextOptions(*builder, widgetOptions,
                                         ...
                                         &f_shadowColor, shadowOffset.width, shadowOffset.height, shadowBlurRadius);
